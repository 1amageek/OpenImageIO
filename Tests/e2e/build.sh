#!/usr/bin/env bash
# Build OIIOSmoke.wasm and stage it next to the HTML/JS loader.
#
...
SDK="${OIIO_SMOKE_SDK:-swift-6.3.1-RELEASE_wasm}"
if [[ -n "${OIIO_SWIFT_BIN:-}" ]]; then
    SWIFT_COMMAND=("$OIIO_SWIFT_BIN")
...
echo "→ Building OIIOSmoke against SDK=$SDK"
cd "$SMOKE_DIR"
"${SWIFT_COMMAND[@]}" package resolve
"${SWIFT_COMMAND[@]}" build --product OIIOSmoke --swift-sdk "$SDK" -c release

BUILT_WASM="$SMOKE_DIR/.build/wasm32-unknown-wasip1/release/OIIOSmoke.wasm"
...
cp "$BUILT_WASM" "$SMOKE_DIR/web/OIIOSmoke.wasm"
cp "$JAVASCRIPTKIT_RUNTIME" "$SMOKE_DIR/web/runtime.mjs"
echo "✓ Staged $(du -h "$SMOKE_DIR/web/OIIOSmoke.wasm" | awk '{print $1}') at Examples/SmokeTest/web/OIIOSmoke.wasm"
