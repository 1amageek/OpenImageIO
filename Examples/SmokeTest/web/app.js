...
function showError(msg) {
    ...
    console.error("[OIIOSmoke]", msg);
}

async function makeWebPFixtureBase64() {
    ...
    const blob = await new Promise((resolve, reject) => {
        canvas.toBlob(
            (result) => result ? resolve(result) : reject(new Error("WebP canvas encode failed")),
            "image/webp",
            0.95
        );
    });
...
async function main() {
    try {
        ...
        const response = await fetch("OIIOSmoke.wasm");
        if (!response.ok) {
            throw new Error("Failed to load OIIOSmoke.wasm: " + response.status);
        }
        ...
        globalThis.__oiio_webp_fixture_base64 = await makeWebPFixtureBase64();
